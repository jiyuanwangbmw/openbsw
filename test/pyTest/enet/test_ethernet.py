...
class TestEthernetNetwork:
    ...
    def test_ping(self, target_session):
        assert target_session.capserial().wait_for_boot_complete()
        response = os.system(
            f"ping -c 2 -W 2 {target_session.target_ip_address()} > /dev/null 2>&1"
        )
        TestEthernetNetwork.ping_success = response == 0
        assert (
            TestEthernetNetwork.ping_success
        ), "Ping failed, Check Ethernet device connection"

    # Test for UDP - Send n (= num_packets) packets and
    # check if the target demo app echoes back the sent data.
    def test_throughput_UDP(self, target_session):
        assert target_session.capserial().wait_for_boot_complete()
        assert (
            TestEthernetNetwork.ping_success
        ), "Ping failed, Check Ethernet device connection"
    ...
    # check if the target demo app echoes back the sent data.
    def test_throughput_TCP(self, target_session):
        assert target_session.capserial().wait_for_boot_complete()
        assert (
            TestEthernetNetwork.ping_success
        ), "Ping failed, Check Ethernet device connection"
